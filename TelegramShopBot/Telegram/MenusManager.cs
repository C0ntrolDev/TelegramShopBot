...
namespace TelegramShopBot
{
    internal class MenusManager
    {
        ...
        private AccountMenu accountMenu = new("👤Аккаунт👤");
        ...
        {
            categoriesMenu = new CategoriesMenu("📚Все категории📚", maxKeyboardLineLenght);

            allMenus = new()
            {
                categoriesMenu,
                accountMenu,
                new SimpleMenu("❔Помощь❔", Tools.GetMessageTextFromTxt("HelpMessage.txt")),
                new SimpleMenu("🛑Правила🛑", Tools.GetMessageTextFromTxt("RulesMessage.txt")),
                new SimpleMenu("💻Коментарий от разработчика💻", Tools.GetMessageTextFromTxt("DeveloperMessage.txt"))
            };
